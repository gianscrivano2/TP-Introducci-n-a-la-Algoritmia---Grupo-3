def lista_clientes(clientes, cant_horas, precio, precio_promedio):
    ...
    while dni!=-1:
        if dni>=5000000 and dni<=99000000: #Chequea que DNI este dentro de los valores aceptados
            ...
            if repetido:
                ...
                clientes.append(dni)
                cant_horas, precio, precio_promedio=horas(cant_horas, precio, precio_promedio)
                dni=int(input("Ingrese DNI o -1 para finalizar: "))
        else:
...
def horas(cant_horas, precio, precio_promedio): #Ingresa cantidad de horas que se quedo en el estacionamiento
    ...
    precio, precio_promedio=valores(hrs, precio, precio_promedio) #Llama a la funcion "valores" para calcular el precio y el promedio por hora

    return cant_horas, precio, precio_promedio

def valores(horas, precio, precio_promedio): #Asigna valor dependiendo de cuantas horas se quedó
    
    if horas==1:
        valor=300
    elif horas>1 and horas<5:
        valor=(horas*200)+100 #El +100 es xq la primera hora sale 300
    else:
        valor=800
    precio.append(valor) #Añade el valor a la lista precio 
    prom_por_hora=valor/horas #Calcula el promedio
